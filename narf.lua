...
local shift = false
local is_armed = false     -- Toggle for MIDI programming
...
function init()
  ...
  -- Initialize 4 tracks with 99 empty steps
  for i = 1, 4 do
    tracks[i] = {
      active_step = 1,
      is_running = false,
  ...
  -- Main MIDI Event Input
  m.event = function(data)
    local msg = midi.to_msg(data)
    if not is_armed then return end -- Ignore if not in Armed mode

    local s = tracks[selected_track].steps[edit_focus]

    -- Programming logic for Keyboard
    if msg.type == "note_on" then
      s.pitch = msg.note
      s.vel = msg.vel
      redraw()
    -- Programming logic for Faders/Knobs
  ...
  -- Track-specific parameter groups
  for i = 1, 4 do
...
function handle_midi_programming(cc, val, s)
  -- 16n Faderbank (Standard range 32-47)
  if cc >= 32 and cc <= 42 then
    update_step_param(s, cc - 31, val)
  -- NanoKontrol2 Faders (0-7)
  elseif cc >= 0 and cc <= 7 then
    update_step_param(s, cc + 1, val)
  -- NanoKontrol2 Knobs (16-18)
  elseif cc >= 16 and cc <= 18 then
    update_step_param(s, cc - 7, val)
  end
end
...
function update_step_param(s, idx, val)
  ...
  elseif idx == 11 then s.prob = math.floor(util.linlin(0, 127, 0, 100, val))
  end
end
...
function run_track(t_idx)
  ...
  while t.is_running do
    ...
    if s.artic < 1.0 then
      clock.sleep(total_sleep * s.artic)
      m:note_off(final_pitch, 0, t.midi_ch)
      clock.sleep(total_sleep * (1 - s.artic))
    else
      clock.sleep(total_sleep)
      m:note_off(final_pitch, 0, t.midi_ch)
    end
...
function load_sequence(slot)
  ...
  if util.file_exists(p) then
    local sd = tab.load(p)
    for i=1,4 do tracks[i].steps = sd[i] end
  end
  redraw()
end
...
function redraw()
  ...
  -- Status Indicators
  if is_armed then
    screen.level(15); screen.move(2, 25); screen.text("● REC")
  end
